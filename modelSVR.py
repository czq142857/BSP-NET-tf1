...
class BSP_SVR(object):
	...
	def build_model(self):
		...
		#for test
		self.point_coord = tf.placeholder(shape=[1,None,4], dtype=tf.float32, name="point_coord")
		self.plane_m = tf.placeholder(shape=[1,4,self.p_dim], dtype=tf.float32, name="plane_m")
		
		self.E_z = self.img_encoder(self.view, phase_train=True, reuse=False)
		self.sE_z = self.img_encoder(self.view, phase_train=False, reuse=True)
		self.sE_m = self.decoder(self.sE_z, phase_train=False, reuse=False)
		self.zE_m = self.decoder(self.z_vector, phase_train=False, reuse=True)
		self.zG, self.zG_max, self.zG2, self.cw2, _ = self.generator(self.point_coord, self.plane_m, phase_train=False, reuse=False)
		...
		self.loss = tf.reduce_mean(tf.square(self.z_vector - self.E_z))
		
		self.saver = tf.train.Saver(max_to_keep=10)

	def generator(self, points, plane_m, phase_train=True, reuse=False):
		with tf.variable_scope("generator") as scope:
			if reuse:
				scope.reuse_variables()
			
			#level 1
			h1 = tf.matmul(points, plane_m)
			h1 = tf.maximum(h1, 0)
	...
	def img_encoder(self, view, phase_train=True, reuse=False):
		with tf.variable_scope("img_encoder") as scope:
			...
			layer_0 = conv2d_nobias(1-view, shape=[7, 7, 1, self.img_ef_dim], strides=[1,2,2,1], scope='conv0')
			layer_0 = lrelu(layer_0)
			#no maxpool
			
			layer_1 = resnet_block(layer_0, self.img_ef_dim, self.img_ef_dim, 'conv1')
			layer_2 = resnet_block(layer_1, self.img_ef_dim, self.img_ef_dim, 'conv2')
			
			layer_3 = resnet_block(layer_2, self.img_ef_dim, self.img_ef_dim*2, 'conv3')
			layer_4 = resnet_block(layer_3, self.img_ef_dim*2, self.img_ef_dim*2, 'conv4')
			
			layer_5 = resnet_block(layer_4, self.img_ef_dim*2, self.img_ef_dim*4, 'conv5')
			layer_6 = resnet_block(layer_5, self.img_ef_dim*4, self.img_ef_dim*4, 'conv6')
			
			layer_7 = resnet_block(layer_6, self.img_ef_dim*4, self.img_ef_dim*8, 'conv7')
			layer_8 = resnet_block(layer_7, self.img_ef_dim*8, self.img_ef_dim*8, 'conv8')
			
			layer_9 = conv2d(layer_8, shape=[4, 4, self.img_ef_dim*8, self.img_ef_dim*16], strides=[1,2,2,1], scope='conv9')
			layer_9 = lrelu(layer_9)
			
			layer_10 = conv2d(layer_9, shape=[4, 4, self.img_ef_dim*16, self.img_ef_dim*16], strides=[1,1,1,1], scope='conv10', padding="VALID")
			layer_10 = tf.reshape(layer_10, [-1,self.img_ef_dim*16])
			layer_10 = lrelu(layer_10)

			l1 = linear(layer_10, self.img_ef_dim*16, scope='l1')
			l1 = lrelu(l1)

			l2 = linear(l1, self.img_ef_dim*16, scope='l2')
			l2 = lrelu(l2)

			l3 = linear(l2, self.img_ef_dim*16, scope='l3')
			l3 = lrelu(l3)
	...
	def decoder(self, zs, phase_train=True, reuse=False):
		with tf.variable_scope("decoder") as scope:
			if reuse:
				scope.reuse_variables()

			l1 = linear(zs, self.ef_dim*16, scope='linear_1')
			l1 = lrelu(l1)
			...
			l4 = linear(l3, self.p_dim*4, scope='linear_4')
